...
function cleanupAndDeactivate() {
    ...
    vscode.window
        .showInformationMessage('Thank you for using File insights! We appreciate your feedback. Would you like to rate the extension?', 'Rate Extension')
        .then((selection) => {
        if (selection === 'Rate Extension') {
            vscode.env.openExternal(vscode.Uri.parse('https://marketplace.visualstudio.com/items?itemName=vscode-samples.status-ts&ssr=false#review-details'));
        }
    });
}
function getFileSize() {
    const fileSize = (0, fs_extra_1.statSync)(vscode.window.activeTextEditor?.document.uri.fsPath || '').size.toString();
